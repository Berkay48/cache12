...
client.on('message', async message => {
  ...
  if(message.content === '.tag') {
    message.channel.send(`İsminize **Cache**, **cache** veya **⁸⁹**, etiketinize de **#0089** alırsanız ailemize katılabilirsiniz.`)//Youtube Matthe
  }
  })
...
client.on("guildMemberAdd", member => {
    ...
    let user = client.users.cache.get(member.id);//Youtube Matthe
    require("moment-duration-format");
...
client.on("userUpdate", async function(oldUser, newUser) { 
    ...
    const tag2 = (ayarlar.tag2)
    const tag3 = (ayarlar.tag3)
